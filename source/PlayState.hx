...
class PlayState extends MusicBeatState
{
	...
	//WELCOME TO THE STUPID BULLSHIT ZONE

	public static var instance:PlayState = null;

	//HOPE YOU ENJOYED YOUR STAY AT THE STUPID BULLSHIT ZONE
	...
	private var strippedCam:FlxCamera;
	...
	public var stripped:Bool = false;

	public var isUghEasterEgg:Bool = false;
	...
	var focusedOnGf:Bool = false;

	public var foregroundSprites = new FlxTypedGroup<FlxSprite>();
	public var midgroundSprites = new FlxTypedGroup<FlxSprite>();
	...
	var gfGraphicMidpointX:Float = 0;
	var gfGraphicMidpointY:Float = 0;

	public var speedTheScroll:Float = 1;

	override public function new(ughState = false)
	{
		super();
		instance = this;
		stripped = ughState;
		isUghEasterEgg = ughState;
	}
	...
	override public function create()
	{
		...
		FlxG.cameras.add(strippedCam);

		var strippedOverlay = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		strippedOverlay.scrollFactor.set();
		strippedOverlay.camera = strippedCam;
		if(stripped)
			{
				add(strippedOverlay);
			}
		...
		speedTheScroll = SONG.speed;
		//FuckYouHaxeFlixel.daRealScrollinSpeed = speedTheScroll;
		...
		enabledEasterEggs = CoolUtil.coolTextFile(Paths.txt('enabledEasterEggs'));
		...
		if(SONG.song.toLowerCase() == 'ugh' || SONG.song.toLowerCase() == 'guns' || SONG.song.toLowerCase() == 'stress' || SONG.song.toLowerCase() == 'ugh-easter-egg')
			{
				hasDialogueScene = false;
			}
		
		if(SONG.song.toLowerCase() == 'philly nice' || SONG.song.toLowerCase() == 'winter horrorland' || SONG.song.toLowerCase() == 'satin panties' || SONG.song.toLowerCase() == 'dad battle' || SONG.song.toLowerCase() == 'ugh' || SONG.song.toLowerCase() == 'guns' || SONG.song.toLowerCase() == 'stress' || SONG.song.toLowerCase() == 'ingame-scene-test' || SONG.song.toLowerCase() == 'ugh-easter-egg')
			{
		...
		switch(SONG.stage)
		{
			case 'halloween': 
			{
				...
				switch(SONG.song.toLowerCase()){
					case 'spookeez' | 'south':
						hallowTex = Paths.getSparrowAtlas('halloween/halloween_bg_withmonster');
					default:
						hallowTex = Paths.getSparrowAtlas('halloween/halloween_bg');
				}
			...
			case 'philly': 
			{
				...
				trace('train sound added to flxg sound list');
			...
			case 'limo':
			{
					...
					if (!FlxG.save.data.effects){
						...
						for (i in 0...5)
						{
								var dancer:BackgroundDancer = new BackgroundDancer((370 * i) + 130, bgLimo.y - 400, songIsMilf);
								dancer.scrollFactor.set(0.4, 0.4);
								grpLimoDancers.add(dancer);
								trace('dancer ' + i + ' added to group');
						}
					}
					...
					if(songIsMilf)
						{
							limoTex = Paths.getSparrowAtlas('limo/limoDriveNight');
						}
					else
						{
							limoTex = Paths.getSparrowAtlas('limo/limoDrive');
						}
					...
					midgroundSprites.add(limo);
					fastCar = new FlxSprite(-300, 160).loadGraphic(Paths.image('limo/fastCarFfs'));
			}
			case 'mall':
			{
						curStage = 'mall';
					...
					if (!FlxG.save.data.effects)
					{
						...
						foregroundSprites.add(santa);
						trace('santa added to fg sprite queue');
					}
			...
			case 'school':
			{
					...
					midgroundSprites.add(bgTrees);
					trace('fg trees added to mg sprite queue');
					...
					foregroundSprites.add(treeLeaves);
					trace('tree leaves added to fg sprite queue');
			...
			case 'schoolEvil':
			{
					...
					bg.scale.set(6, 6);
					add(bg);
					trace('animated evil school added');
			...
			case 'test':
				{
						...
						foregroundSprites.add(stageLightLeft);
						trace('left light added to fg queue');
						...
						foregroundSprites.add(stageLightRight);
						trace('right light added to fg queue');
						...
						foregroundSprites.add(stageCurtains);
						trace('curtains added to fg queue');
				}
			case 'tank':
				{
					defaultCamZoom = .9;
	...
	if(gfVersion == 'pico-speaker')
		...
		if(gfVersion == 'gf-pixel')
			{
				gfGraphicMidpointX = gf.getGraphicMidpoint().x * 2;
				gfGraphicMidpointY = gf.getGraphicMidpoint().y * 2;
			}
		else
			{
				gfGraphicMidpointX = gf.getGraphicMidpoint().x;
				gfGraphicMidpointY = gf.getGraphicMidpoint().y;
			}
		...
		if(!isUghEasterEgg)
			{
				switch (SONG.player2)
				{
					...
					case "monster":
						dad.y += 100;
						trace('monster positioned');
					case 'monster-christmas':
						dad.y += 130;
						trace('monster-christmas positioned');
					case 'dad':
						camPos.x += 400;
						trace('dad positioned');
					case 'pico':
						camPos.x += 600;
						dad.y += 300;
						trace('pico positioned');
					case 'parents-christmas':
						dad.x -= 500;
						trace('parents-christmas positioned');
					case 'senpai':
						dad.x += 150;
						dad.y += 360;
						camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
						trace('senpai positioned');
					case 'senpai-angry':
						dad.x += 150;
						dad.y += 360;
						camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
					...
					case 'spirit':
						...
						dad.y += 100;
						camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
						trace('spirit positioned');
					case 'spirit-flash':
						dad.x -= 150;
						dad.y += 100;
						camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
						trace('spirit-flash positioned');
				}
			}
		if(curStage == 'tankmen')
			{
		...
		trace('added foreground sprites group');
		add(foregroundTankmen);
		trace('added foreground tankmen group');
				...
				case 'roses':
					if(hasDialogueScene)
					...
					else if(hasVideoScene)
						{
							...
							//FOR NOW JUST GONNA
							startCountdown();
							trace('video launched');
						}
					else
						{
							startCountdown();
							trace('countdown launched');
						}
				//commented out cuz dupe of default
				//case 'thorns':
				//	schoolIntro(doof);
	...
	function tweenCamInGfSad():Void
		{
			FlxTween.tween(FlxG.camera, {zoom: 1.3}, 0.5, {ease: FlxEase.elasticInOut});
		}
	...
	override public function update(elapsed:Float)
	{
		...
		//this code made the game piss and shit itself
		/*
		if(enabledEasterEggs.contains('ascend'))
			{
				if(SONG.notes[Std.int(curStep / 16)].mustHitSection)
					{
						boyfriend.y -= 50 * elapsed;
					}
				else
					{
						dad.y -= 50 * elapsed;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (FlxG.save.data.downscroll)
						daNote.y = (strumLine.y - (Conductor.songPosition - daNote.strumTime) * (-0.45 * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? speedTheScroll : FlxG.save.data.scrollSpeed, 2)));
					else
						daNote.y = (strumLine.y - (Conductor.songPosition - daNote.strumTime) * (0.45 * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? speedTheScroll : FlxG.save.data.scrollSpeed, 2)));
					//trace(daNote.y);
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (combo > 5)
			{
				...
				focusedOnGf = true;
				camFollow.setPosition(gfGraphicMidpointX, gfGraphicMidpointY);
				tweenCamInGfSad();
	...
	*/
	...
	function fastCarDrive()
	{
		...
		FlxTween.tween(FlxG.camera, {zoom: 1.15}, 0.25, {ease: FlxEase.expoOut,});
	...
	function lightningStrikeShit():Void
	{
		if (!FlxG.save.data.effects){
			...
			FlxTween.tween(FlxG.camera, {zoom: 1.15}, 0.3, {ease: FlxEase.expoOut,});
		}
	}
	...
	override function stepHit()
	{
				...
				if(curStep == 92 || curStep == 156 || curStep == 252 || curStep == 348 || curStep == 412)
					{
						...
						speedTheScroll = SONG.speed + 0.3;
						for(existingNote in notes)
							{
								existingNote.reset(existingNote.x, existingNote.y);
								//reset the notes to the new scroll speed
							}
					}
		...
		if(SONG.song.toLowerCase() == 'ugh' || SONG.song.toLowerCase() == 'ugh-easter-egg')
			{
		...
		if(SONG.song.toLowerCase() == 'stress')
			{
				//SO THE OPTION DOESN'T MUTE THE "HUH PRETTY GOOD"
				if(curStep == 736 || curStep == 757)
					{
						canMuteVox = false;
					}

				if(curStep == 741 || curStep == 765)
					{
						canMuteVox = true;
					}
				if(curStep == 736)
					{
						altAnim = '-alt';
						FlxTween.tween(FlxG.camera, {zoom: 1.25}, 2.15, {ease: FlxEase.expoOut,});
						speedTheScroll = SONG.speed + 0.3;
						for(existingNote in notes)
							{
								existingNote.reset(existingNote.x, existingNote.y);
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			{
				speedTheScroll = SONG.notes[Math.floor(curStep / 16)].scrollSpeed;
				for(existingNote in notes)
					{
						existingNote.reset(existingNote.x, existingNote.y);
						//reset the notes to the new scroll speed
		...
		// HARDCODING FOR MILF ZOOMS AND SCROLL SPEED CHANGE
		if (curSong.toLowerCase() == 'milf' && curBeat >= 168 && curBeat < 200 && camZooming && FlxG.camera.zoom < 1.35)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
			speedTheScroll = SONG.speed + 0.4;
			for(existingNote in notes)
				{
					existingNote.reset(existingNote.x, existingNote.y);
					//reset the notes to the new scroll speed
