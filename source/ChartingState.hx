...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var leftIcon:HealthIconCharting;
	var rightIcon:HealthIconCharting;
	...
	override function create()
	{
		...
		*/
		...
		leftIcon = new HealthIconCharting(_song.player1);
		rightIcon = new HealthIconCharting(_song.player2);
	...
	var check_changeScrollSpeed:FlxUICheckBox;
	var stepperSectionBPM:FlxUINumericStepper;
	var stepperSectionScrollSpeed:FlxUINumericStepper;
	...
	function addSectionUI():Void
	{
		...
		stepperSectionScrollSpeed = new FlxUINumericStepper(10, 260, 1, _song.speed, 0, 999, 0);
		stepperSectionScrollSpeed.value = _song.speed;
		stepperSectionScrollSpeed.name = 'section_scrollspeed';
		...
		check_changeScrollSpeed = new FlxUICheckBox(10, 300, null, null, "Change Scroll Speed", 100);
		check_changeScrollSpeed.name = 'check_changeScrollSpeed';
		...
		tab_group_section.add(stepperSectionScrollSpeed);
		tab_group_section.add(stepperCopy);
		tab_group_section.add(stepperCopyLabel);
		tab_group_section.add(check_mustHitSection);
		tab_group_section.add(check_altAnim);
		tab_group_section.add(check_changeBPM);
		tab_group_section.add(check_changeScrollSpeed);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Change Scroll Speed':
					_song.notes[curSection].changeScrollSpeed = check.checked;
					FlxG.log.add('changed scroll speed shit');
			}
		}
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch (wname)
			{
				...
				case 'section_scrollspeed':
					if (nums.value <= 0.1)
						nums.value = 0.1;
					_song.notes[curSection].scrollSpeed = Std.int(nums.value);
			}
		}
		...
		*/
		...
		curRenderedNotes.forEach(function(note:Note)
			{
				if (strumLine.overlaps(note) && pressArray[Math.floor(Math.abs(note.noteData))])
				{
					deleteNote(note);
					delete = true;
					trace('delete note at step ' + curStep + ' in section ' + curSection);
				}
			});
		...
		if (!typingShit.hasFocus)
		{
			...
			if (FlxG.keys.pressed.CONTROL)
			{
				...
				{
					trace(curRenderedNotes.members.contains(lastNote) ? "delete note " : "add note " + "at " + lastNote.strumTime);
	...
	function updateSectionUI():Void
	{
		...
		check_changeScrollSpeed.checked = sec.changeScrollSpeed;
		stepperSectionBPM.value = sec.bpm;
		stepperSectionScrollSpeed.value = sec.scrollSpeed;
	}
	...
	function onSaveComplete(_):Void
	{
		...
		_file = null;
		FlxG.log.notice("Successfully saved chart of song " + _song.song + " at " + Date.now() + ".");
		trace("Successfully saved chart of song " + _song.song + " at " + Date.now() + ".");
	}
	...
	function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Problem saving chart of song " + _song.song + " at " + Date.now() + ".");
		trace("Problem saving chart of song " + _song.song + " at " + Date.now() + ".");
	}
}